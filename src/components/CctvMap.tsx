...
import { Compass, MapPin, Layers, Train, Globe, Moon, Sun, Activity } from 'lucide-react';
...
export const CctvMap: React.FC<CctvMapProps> = ({
  ...
  const [railSpeedMultiplier, setRailSpeedMultiplier] = useState<number>(1);
  const speedMultiplierRef = useRef<number>(1);

  useEffect(() => {
    speedMultiplierRef.current = railSpeedMultiplier;
  }, [railSpeedMultiplier]);
    ...
    const loadRailData = async () => {
      try {
        ...
        interface ActiveTrain {
          ...
          direction: number;
          speedKmH: number;
          type: 'thsr' | 'tra_express' | 'tra_local';
        }

        const animatedTrains: ActiveTrain[] = [];
          ...
          if (thsrData.lines && Array.isArray(thsrData.lines)) {
            thsrData.lines.forEach((line: any) => {
              if (line.shape && Array.isArray(line.shape) && line.shape.length > 5) {
                ...
                const totalKm = cum[cum.length - 1];
                L.polyline(latlngs, { color: '#f97316', weight: 4, opacity: 0.85, dashArray: '8, 6' }).bindTooltip('🚄 台灣高鐵 (THSR 全線 350km)', { sticky: true }).addTo(railGroup);
                ...
                thsrTrainConfigs.forEach(cfg => {
                  const thsrIcon = L.divIcon({ className: 'custom-train-thsr', html: `<div class="w-6 h-6 rounded-full bg-gradient-to-tr from-orange-600 to-amber-500 border-2 border-white shadow-[0_0_14px_#f97316] flex items-center justify-center text-[10px] text-white animate-pulse">🚄</div>`, iconSize: [24, 24], iconAnchor: [12, 12] });
                  const { pos } = interpolatePosAlongTrack(latlngs, cum, cfg.startKm);
                  const marker = L.marker(pos, { icon: thsrIcon }).bindTooltip(`<div class="p-1 font-sans text-xs font-bold text-orange-400">${cfg.name}</div>`).addTo(railGroup);
                  animatedTrains.push({ marker, name: cfg.name, path: latlngs, cum, totalKm, curDistKm: cfg.startKm, direction: cfg.dir, speedKmH: cfg.speed, type: 'thsr' });
                });
              }
            });
          }
        ...
        if (traRes && traRes.ok) {
          ...
          if (traData.lines && Array.isArray(traData.lines)) {
            const traTrainNames = [
              { name: '🚆 台鐵 EMU3000', speed: 125, isExpress: true },
            ...
            traData.lines.forEach((line: any) => {
              if (line.shape && Array.isArray(line.shape) && line.shape.length > 10) {
                ...
                const cum = computeTrackCumulativeKm(latlngs);
                const totalKm = cum[cum.length - 1];
                L.polyline(latlngs, { color: line.color || '#06b6d4', weight: 2.5, opacity: 0.75 }).bindTooltip(`台鐵 ${line.name}`, { sticky: true }).addTo(railGroup);
                if (totalKm > 15 && trainCount < traTrainNames.length) {
                  ...
                  const traIcon = L.divIcon({ className: 'custom-train-tra', html: `<div class="w-6 h-6 rounded-full ${cfg.isExpress ? 'bg-cyan-600' : 'bg-teal-600'} border-2 border-white flex items-center justify-center text-[10px] text-white animate-pulse">🚆</div>`, iconSize: [24, 24], iconAnchor: [12, 12] });
                  const { pos } = interpolatePosAlongTrack(latlngs, cum, startKm);
                  const marker = L.marker(pos, { icon: traIcon }).bindTooltip(`<div class="p-1 font-sans text-xs font-bold text-cyan-400">${cfg.name}</div>`).addTo(railGroup);
                  animatedTrains.push({ marker, name: cfg.name, path: latlngs, cum, totalKm, curDistKm: startKm, direction: trainCount % 2 === 0 ? 1 : -1, speedKmH: cfg.speed, type: cfg.isExpress ? 'tra_express' : 'tra_local' });
                }
              }
            });
          }
        ...
        const animate = (now: number) => {
          ...
          animatedTrains.forEach(t => {
            ...
            if (t.curDistKm >= t.totalKm) { t.curDistKm = t.totalKm; t.direction = -1; }
            else if (t.curDistKm <= 0) { t.curDistKm = 0; t.direction = 1; }
        ...
        animReq = requestAnimationFrame(animate);
      } catch (err) { console.warn('Rail animation error:', err); }
    };
    loadRailData();
    return () => { if (animReq) cancelAnimationFrame(animReq); };
  ...
  useEffect(() => {
    ...
    existingIds: for (const [id, marker] of markersRef.current) {
      if (!filteredIds.has(id)) { clusterGroup.removeLayer(marker); markersRef.current.delete(id); }
    }
    ...
    filteredCctvs.forEach(cctv => {
      ...
      const marker = L.marker([cctv.latitude, cctv.longitude], { icon: L.icon({ iconUrl: iconUrl(iconFile), iconSize: [32, 32], iconAnchor: [16, 32] }) });
      ...
      const popupHtml = `
        <div class="p-3 w-64 text-slate-100 font-sans">
          <div class="flex items-center justify-between gap-2 mb-2">
            ...
            <span class="px-2 py-0.5 rounded text-[10px] font-semibold ${cctv.status === 'online' ? 'bg-emerald-500/20 text-emerald-400 border-emerald-500/30' : cctv.status === 'unstable' ? 'bg-amber-500/20 text-amber-400 border-amber-500/30' : 'bg-rose-500/20 text-rose-400 border-rose-500/30'}">
          ...
          <div class="aspect-video bg-slate-950 rounded border border-slate-800 mb-3 overflow-hidden">
            <img src="${cctv.snapshotUrl || placeholderSvg}" class="w-full h-full object-cover" onerror="this.src='${placeholderSvg}'" />
          </div>
          <div class="grid grid-cols-2 gap-1.5">
            <button id="btn-play-${cctv.cctvId}" class="bg-blue-600 text-white py-1.5 rounded text-xs">觀看影像</button>
            <button id="btn-check-${cctv.cctvId}" class="bg-slate-700 text-white py-1.5 rounded text-xs">測試</button>
          </div>
        </div>
      `;
      marker.bindPopup(popupHtml).on('popupopen', () => {
        document.getElementById(`btn-play-${cctv.cctvId}`)!.onclick = () => onSelectCctv(cctv);
        document.getElementById(`btn-check-${cctv.cctvId}`)!.onclick = () => onCheckStatus(cctv.cctvId);
      });
  ...
  return (
    <div className="relative w-full h-[calc(100vh-160px)] min-h-[520px] bg-slate-950 rounded-2xl overflow-hidden border border-slate-800/80 shadow-2xl flex flex-col">
      ...
      <div className="absolute top-3 left-3 z-[1000] flex flex-wrap items-center gap-2 max-w-[calc(100%-60px)]">
        ...
        {/* Rail & Animated Train Layer Toggle & Multiplier */}
        <div className="bg-slate-900/90 backdrop-blur-md border border-slate-800 rounded-xl p-1.5 flex items-center gap-1.5 shadow-2xl text-xs">
          ...
          >
            ...
            <span>🚆 鐵道動態 {showRailLayer ? '開啟' : '關閉'}</span>
          </button>
          ...
          {showRailLayer && (
            <div className="flex items-center gap-1 bg-slate-950/90 px-1.5 py-0.5 rounded-lg border border-slate-800">
              ...
              {[1, 3, 5].map(m => (
                <button
                  key={m}
                  onClick={() => setRailSpeedMultiplier(m)}
                  className={`px-1.5 py-0.5 rounded text-[10px] font-mono transition ${
                    railSpeedMultiplier === m ? 'bg-orange-500 text-white font-bold' : 'text-slate-400 hover:text-white'
                  }`}
                >
                  {m}x
                </button>
      ...
      {/* Map Bottom Legend Overlay (Three-tier Health) */}
